...
namespace Unity.Collections
{
    ...
    public struct NativeHeap<T, U> : IDisposable where T : unmanaged where U : unmanaged, IComparer<T>
    {
            ...
            set
            {
                unsafe
                {
...
#endif
                    var newTable = (TableValue*)Malloc(SizeOf<TableValue>() * value, AlignOf<TableValue>(), Allocator);
                    var newHeap = (HeapNode<T>*)Malloc(SizeOf<HeapNode<T>>() * value, AlignOf<HeapNode<T>>(), Allocator);
                    ...
                    for (var i = 0; i < value - Data->Capacity; i++)
                    {
                        ...
                        newHeap[i + Data->Capacity] = new HeapNode<T> { TableIndex = i + Data->Capacity };

#if ENABLE_UNITY_COLLECTIONS_CHECKS
                        //For each new table value, make sure it has a specific version
                        newTable[i + Data->Capacity] = new TableValue { Version = 1 };
#endif
                    }
        ...
        internal unsafe NativeHeap(int initialCapacity, U comparator, Allocator allocator, int disposeSentinelStackDepth)
        {
            ...
            if (allocator == Allocator.None || allocator == Allocator.Invalid || allocator == Allocator.AudioKernel)
            {
            ...
            {
                Data->Heap[i] = new HeapNode<T> { TableIndex = i };
#if ENABLE_UNITY_COLLECTIONS_CHECKS
                Data->Table[i] = new TableValue { Version = 1 };
#endif
            }
            ...
            switch (errorCode)
            {
                case VALIDATION_ERROR_WRONG_INSTANCE:
                    throw new ArgumentException(
                        "The provided ItemHandle was not valid for this NativeHeap.  " + "It was taken from a different instance.");
                ...
                case VALIDATION_ERROR_REMOVED:
                    throw new ArgumentException(
                        "The provided ItemHandle was not valid for this NativeHeap.  " + "The item it pointed to might have already been removed.");
            }
#endif
        }
    ...
    internal unsafe class NativeHeapDebugView<T, U> where T : unmanaged where U : unmanaged, IComparer<T>
    {
    ...
    [StructLayout(LayoutKind.Sequential)]
    internal unsafe struct HeapData<T, U> where T : unmanaged
    {
        public int Count;

        public int Capacity;

        public HeapNode<T>* Heap;
