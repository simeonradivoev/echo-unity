// Copy from unity UI and modified. It's a mess needs to be cleaned up.
...
{
    /// <summary>
    /// Input module that takes its input from
    ...
    public class InputSystemXRINputModule : BaseInputModule
    {
        /// <summary>
        ...
        private static readonly Dictionary<InputAction, InputActionReferenceState> s_InputActionReferenceCounts = new();

        [SerializeField]
        private float _pressThreshold;

        [SerializeField]
        private float _pressThresholdLeeway;
        ...
        private InlinedArray<InputControl> m_PointerTouchControls;

        /// <summary>
        /// Whether to clear the current selection when a click happens that does not hit any
        /// <c>
        /// GameObject
        /// </c>
        /// .
        ...
        private bool shouldIgnoreFocus =>
            ...
            // react.
            explictlyIgnoreFocus || Application.runInBackground;
        ...
        /// </summary>
        /// <remarks>
        /// This will transform all tracked pointers. If unset, or set to null, the Unity world origin will be used as the basis for all tracked positions and
        /// rotations.
        /// </remarks>
        ...
        /// <summary>
        /// Scales the drag threshold of
        /// <c>
        /// EventSystem
        /// </c>
        /// for tracked devices to make selection easier.
        /// </summary>
        [field: Tooltip("Scales the Eventsystem.DragThreshold, for tracked devices, to make selection easier.")]
        public float trackedDeviceDragThresholdMultiplier { get; set; } = 2.0f;
        ...
        /// <see cref="InputAction.expectedControlType"/> set to
        /// <c>
        /// "Vector2"
        /// </c>
        /// .
        ...
        /// <summary>
        /// An <see cref="InputAction"/> delivering a
        /// <c>
        /// Vector2
        /// </c>
        /// scroll wheel value
        ...
        /// float
        /// </c>
        /// button value that determines when
        /// <c>
        /// ISubmitHandler
        /// </c>
        /// is triggered.
        ...
        /// Quaternion
        /// </c>
        /// value reflecting the orientation of <see cref="TrackedDevice"/>s.
        ...
        /// Vector3
        /// </c>
        /// value reflecting the position of <see cref="TrackedDevice"/>s.
        ...
        public float PressThreshold => _pressThreshold;
        ...
        /// <summary>
        /// Check whether the given pointer or touch is currently hovering over a
        /// <c>
        /// GameObject
        /// </c>
        /// .
        /// </summary>
        /// <param name="pointerOrTouchId">
        /// ID of the pointer or touch. Meaning this should correspond to either
        /// <c>
        /// PointerEventData.pointerId
        /// </c>
        /// or <see cref="ExtendedPointerEventData.touchId"/>. The pointer ID
        ...
        /// and the touch was over a
        /// <c>
        /// GameObject
        /// </c>
        /// , the associated pointer is still considered over that
        /// object for the frame in which the touch ended.
        /// 
        /// To check whether any pointer is over a
        /// <c>
        /// GameObject
        /// </c>
        /// , simply pass a negative value such as -1.
        /// </param>
        /// <returns>
        /// True if the given pointer is currently hovering over a
        /// <c>
        /// GameObject
        /// </c>
        /// .
        /// </returns>
        /// <remarks>
        /// The result is true if the given pointer has caused an
        /// <c>
        /// IPointerEnter
        /// </c>
        ...
        /// .
        /// 
        /// Be aware that this method relies on state set up during UI event processing that happens in
        /// <c>
        /// EventSystem.Update
        /// </c>
        /// ,
        ...
        /// MonoBehaviour
        /// </c>
        /// updates. This step happens
        /// <em>
        /// after
        /// </em>
        /// input processing.
        /// Thus, calling this method earlier than that in the frame will make it poll state from
        /// <em>
        /// last
        /// </em>
        /// frame.
        ...
        /// finger). For touch, you can use the <see cref="TouchControl.touchId"/> of the touch.
        /// 
        /// Negative values will return an invalid <see cref="RaycastResult"/>.
        /// </param>
        /// <returns>
        ...
        private RaycastResult PerformRaycast(XRExtendedPointerEventData eventData, out float maxDistance)
        {
            if (eventData == null)
            {
            ...
            // the first shot.
            if (eventData.pointerType == UIPointerType.Tracked && XRTrackedDeviceRaycasater.s_Instances.length > 0)
            {
                for (var i = 0; i < XRTrackedDeviceRaycasater.s_Instances.length; ++i)
                {
                    var trackedDeviceRaycaster = XRTrackedDeviceRaycasater.s_Instances[i];
                    m_RaycastResultCache.Clear();
                    trackedDeviceRaycaster.Raycast(eventData, m_RaycastResultCache);
                    if (m_RaycastResultCache.Count > 0)
                    {
                        var raycastResult = m_RaycastResultCache[0];
                        m_RaycastResultCache.Clear();
                        maxDistance = trackedDeviceRaycaster.maxDistance;
        ...
        private void ProcessPointer(ref PointerModel state)
        {
            ...
            eventData.pointerCurrentRaycast = PerformRaycast(eventData, out var maxDistance);
            ...
            if (pointerType == UIPointerType.Tracked)
            {
                if (eventData.pointerCurrentRaycast.isValid)
                {
                    ...
                    eventData.pressure = Mathf.Clamp01(eventData.pointerCurrentRaycast.distance / maxDistance);

                    if (eventData.firstEnter)
                    {
                        eventData.lastDistance = eventData.pointerCurrentRaycast.distance < 0 ? float.NegativeInfinity : float.PositiveInfinity;
                        eventData.firstEnter = false;
                    }
                    ...
                    if (!state.leftButton.isPressed &&
                        eventData.pointerCurrentRaycast.distance >= 0 &&
                        eventData.distanceDelta > 0 &&
                        eventData.pointerCurrentRaycast.distance <= _pressThreshold)
                    {
                        OnClickCallback(ref state, true, false);
                    }
                }
                else
                {
                    eventData.firstEnter = true;
        ...
        private void ProcessPointerButton(ref PointerModel.ButtonState button, XRExtendedPointerEventData eventData)
        {
            ...
            if (button.wasPressedThisFrame)
            {
                ...
                eventData.pressLocalPos = currentOverGo.transform.InverseTransformPoint(eventData.pointerCurrentRaycast.worldPosition);
            ...
            if (button.wasReleasedThisFrame)
            {
                ...
                eventData.excapedPressLag = false;
            }

            button.CopyPressStateFrom(eventData);
        }

        private void ProcessPointerButtonDrag(ref PointerModel.ButtonState button, XRExtendedPointerEventData eventData)
        {
            ...
            if (!eventData.dragging)
            {
                if (!eventData.useDragThreshold ||
                    ...
                    (eventData.pointerType == UIPointerType.Tracked ? trackedDeviceDragThresholdMultiplier : 1))
                {
        ...
        private bool IsMoveAllowed(AxisEventData eventData)
        {
            ...
            return navigationTarget.transform.IsChildOf(localMultiPlayerRoot.transform);
        }
        ...
        /// </c>
        /// component is programmatically added to a
        /// <c>
        /// GameObject
        /// </c>
        /// ,
        /// it will automatically receive the default actions as part of its
        /// <c>
        /// OnEnable
        /// </c>
        /// method. Use <see cref="UnassignActions"/>
        ...
        private int GetPointerStateIndexFor(InputDevice source, bool createIfNotExists = true)
        {
            var pointerId = source.deviceId;
            ...
            if (m_PointerBehavior == UIPointerBehavior.SingleUnifiedPointer)
            {
                ...
                {
                    m_CurrentPointerIndex = AllocatePointer(pointerId, pointerId, 0, UIPointerType.Tracked, null, null);
                }
                else
                {
                    ...
                    eventData.control = null;
                    eventData.device = source;
                    eventData.pointerType = UIPointerType.Tracked;
                    eventData.pointerId = pointerId;
                    eventData.touchId = 0;
#if UNITY_2022_3_OR_NEWER
                    eventData.displayIndex = pointerId;
#endif
                ...
                m_CurrentPointerId = pointerId;
                m_CurrentPointerType = UIPointerType.Tracked;
            ...
            index = AllocatePointer(pointerId, pointerId, 0, UIPointerType.Tracked, null, source);
        ...
        private int GetPointerStateIndexFor(InputControl control, bool createIfNotExists = true)
        {
            ...
            var displayIndex = GetDisplayIndexFor(control);

            if (touchId != 0)
            {
                pointerId = MakePointerIdForTouch(pointerId, touchId);
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    var trackedDevice = positionControls.HasValue && positionControls.Value.Count > 0 ? positionControls.Value[0].device : null;
            ...
            InputControl touchControl = null)
        {
            ...
            var eventData = default(XRExtendedPointerEventData);
            ...
            if (eventData == null)
            {
                eventData = new XRExtendedPointerEventData(eventSystem);
            }
        ...
        private bool IgnoreNextClick(bool wasPressed, bool wasCanceled)
        {
            ...
            return wasCanceled && !Application.isFocused && wasPressed;
        }
        ...
        private void OnClickCallback(ref PointerModel state, bool pressed, bool canceled)
        {
            var wasPressed = state.leftButton.isPressed;
            state.leftButton.isPressed = pressed;
            state.changedThisFrame = true;
            if (IgnoreNextClick(wasPressed, canceled))
            {
        ...
        private void OnRightClickCallback(InputAction.CallbackContext context)
        {
            ...
            ref var state = ref GetPointerStateForIndex(index);
            var wasPressed = state.rightButton.isPressed;
        ...
        private void OnMiddleClickCallback(InputAction.CallbackContext context)
        {
            ...
            ref var state = ref GetPointerStateForIndex(index);
            var wasPressed = state.middleButton.isPressed;
        ...
        public override int ConvertUIToolkitPointerId(PointerEventData sourcePointerData)
        {
            ...
            return sourcePointerData is ExtendedPointerEventData ep ? ep.uiToolkitPointerId : base.ConvertUIToolkitPointerId(sourcePointerData);
        }
        ...
        private void HookActions()
        {
            ...
            if (m_OnXRPointDelegate == null)
            {
                m_OnXRPointDelegate = OnXRPointDelegate;
            }
